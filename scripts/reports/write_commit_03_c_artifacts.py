#!/usr/bin/env python3
"""Materialize commit-03-c infra-runtime-fake artifacts and report."""
...
def main() -> None:
    ...
    cases = [
        build_case(
            args.run_id,
            "idemp-001-operation-namespace-isolation",
            "TC-ID-IDEMP-001",
            "EV-CAND-ID-IDEMP-001",
            "idemp.operation_namespace.isolated_by_operation_and_channel",
            "Idempotency records stay isolated by operation name and channel even when the raw key is shared.",
        ),
        build_case(
            args.run_id,
            "idemp-002-missing-stored-result-no-recompute",
            "TC-ID-IDEMP-002",
            "EV-CAND-ID-IDEMP-002",
            "idemp.duplicate.missing_stored_result_no_recompute",
            "Duplicate replay surfaces the missing stored result instead of recomputing from current truth.",
        ),
        build_case(
            args.run_id,
            "idemp-003-consumer-duplicate-replays-typed-receipt",
            "TC-ID-IDEMP-003",
            "EV-CAND-ID-IDEMP-003",
            "idemp.consumer_duplicate.typed_receipt_replayed",
            "Consumer duplicate replay returns the stored typed receipt envelope without reapplying the mutation.",
        ),
        build_case(
            args.run_id,
            "idemp-004-job-duplicate-replays-stored-report",
            "TC-ID-IDEMP-004",
            "EV-CAND-ID-IDEMP-004",
            "idemp.job_duplicate.stored_report_replayed",
            "Job duplicate replay loads the stored job report instead of relisting current maintenance targets.",
        ),
        build_case(
            args.run_id,
            "idemp-006-stored-result-precedes-idempotency-complete",
            "TC-ID-IDEMP-006",
            "EV-CAND-ID-IDEMP-006",
            "idemp.stored_result.saved_before_complete",
            "Stored result durability is staged before idempotency completion so completion cannot point at a missing result.",
        ),
        build_case(
            args.run_id,
    ...
    write_json(
        artifact_root / "meta" / "config-digest.json",
        {
            ...
            "config_sources": [],
            "config_reason_ref": "config:not-applicable:commit-03-c-infra-runtime-fake",
