#!/usr/bin/env python3
"""Materialize commit-04-c service-flow-fast and redaction artifacts."""
...
def main() -> None:
    ...
    parser.add_argument("--implementation-source_ref", required=True, dest="implementation_source_ref")
    ...
    service_cases = [
        build_case(
            run_id=args.run_id,
            suite=SERVICE_SUITE,
            case_id="cmd-011-prepare-trace-handoff-accepted",
            tc_refs=["TC-ID-CMD-011"],
            evidence_candidate_refs=["EV-CAND-ID-CMD-011"],
            evidence_refs=["EV-ID-CMD-001"],
            assertion_kind="flow",
            assertion_ref="command.trace_handoff.accepted.pending_only",
            safe_message=(
                "PrepareTraceHandoff persists a pending handoff intent with non-empty trace refs and explicit empty outbox refs, without performing delivery."
            ),
        ),
        build_case(
            run_id=args.run_id,
            suite=SERVICE_SUITE,
            case_id="cmd-012-prepare-trace-handoff-empty-trace-rejected",
            tc_refs=["TC-ID-CMD-012"],
            evidence_candidate_refs=["EV-CAND-ID-CMD-012"],
            evidence_refs=["EV-ID-CMD-001"],
            assertion_kind="flow",
            assertion_ref="command.trace_handoff.empty_trace.rejected",
            safe_message=(
                "Empty trace selections are rejected before any handoff intent, trace, audit, or stored accepted result is saved."
            ),
        ),
        build_case(
            run_id=args.run_id,
            suite=SERVICE_SUITE,
            case_id="cmd-013-command-duplicate-same-digest-replay",
            tc_refs=["TC-ID-CMD-013"],
            evidence_candidate_refs=["EV-CAND-ID-CMD-013"],
            evidence_refs=["EV-ID-CMD-001", "EV-ID-IDEMP-001"],
            assertion_kind="replay",
            assertion_ref="command.trace_handoff.duplicate.replay",
            safe_message=(
                "A same-key same-digest duplicate replays the stored accepted handoff result and effect envelope without creating a second intent or triggering delivery."
            ),
        ),
        build_case(
            run_id=args.run_id,
            suite=SERVICE_SUITE,
            case_id="cmd-014-command-duplicate-different-digest-conflict",
            tc_refs=["TC-ID-CMD-014"],
            evidence_candidate_refs=["EV-CAND-ID-CMD-014"],
            evidence_refs=["EV-ID-CMD-001", "EV-ID-IDEMP-001"],
            assertion_kind="replay",
            assertion_ref="command.duplicate.different_digest.conflict",
            safe_message=(
                "A same-key different-digest command returns the formal duplicate conflict surface and does not overwrite the original stored replay source."
            ),
        ),
        build_case(
            run_id=args.run_id,
            suite=SERVICE_SUITE,
            case_id="cmd-015-command-rollback-conflict-subset",
            tc_refs=["TC-ID-CMD-015"],
            evidence_candidate_refs=["EV-CAND-ID-CMD-015"],
            evidence_refs=["EV-ID-CMD-001", "EV-ID-IDEMP-001"],
            assertion_kind="flow",
            assertion_ref="command.rollback.conflict.subset_consistent",
            safe_message=(
                "Command rollback and conflict subset evidence keeps accepted truth, stored replay, and idempotency completion consistent instead of leaving partial writes behind."
            ),
        ),
    ]
    ...
    redaction_cases = [
        build_case(
            ...
            case_id="cmd-012-trace-handoff-forbidden-material-surface",
            tc_refs=["TC-ID-CMD-012"],
            evidence_candidate_refs=["EV-CAND-ID-CMD-012"],
            evidence_refs=["EV-ID-REDACTION-001"],
            assertion_kind="redaction",
            assertion_ref="redaction.trace_handoff.rejected.before_body_leak",
            safe_message=(
                "Rejected trace handoff inputs stop before any blocked handoff material family is persisted or reported."
            ),
        ),
        build_case(
            run_id=args.run_id,
            ...
            safe_message=(
                "Run-scoped artifacts, reports, and suite logs stay limited to safe refs, bounded labels, and body-free verification text."
            ),
        ),
        build_case(
            run_id=args.run_id,
        ...
        build_case(
            ...
            safe_message=(
                "Logs and metrics remain safe diagnostics only and do not replace accepted command trace, audit, or stored replay evidence."
            ),
        ),
    ]
    ...
    write_json(
        artifact_root / "meta" / "config-digest.json",
        {
            ...
            "config_sources": [],
            "config_reason_ref": (
                "config:not-applicable:commit-04-c-service-flow-fast-redaction"
            ),
